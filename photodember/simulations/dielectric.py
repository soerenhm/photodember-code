...
class DielectricConfig:
    ...
    def create_simulation(self) -> Tuple[Simulation, SimulationState]:
        ...
        excitation_data = self.excitation_data # to_simulation_grid(load_excitation_data(self.excitation_datafile), x)
        electron_hole_density = self.initial_electron_hole_density * np.ones_like(x)
        # masses = self.particle_masses #[self.electron_data.mass, self.hole_data.mass]
        final_temperatures = self.excitation_temperatures #particle_temperatures(excitation_data.energy_per_particle, masses)
        ...
        # initial_electron_temperature = self.phonon_temperature * np.ones_like(x) #temperatures[0]
        # initial_hole_temperature = self.phonon_temperature * np.ones_like(x) #temperatures[1]
