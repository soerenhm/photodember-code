...
surface_electric_field = [st.electric_field[1] * 0.5 for st in states]
...
plt.subplot(2, 1, 2)
t_fs = np.array(t) * 1e15
plt.plot(t_fs, 0.5 * (Esim[:, 0] + Esim[:, 1]) * 1e-7, "g-")
plt.fill_between(
    t_fs,
    1.3
    * np.exp(
        -4
        * np.log(2)
        * ((np.array(t) - conf.excitation_time_zero) / conf.excitation_time_fwhm) ** 2
    ),
    lw=1,
    ls="-",
    color="k",
...
electric_field_t_integral = np.array(
    [
        np.convolve(conv, electric_field[:, i])[: len(t)]
...
def solve_helmholtz(alpha, gam, state_index):
    ...
    k_z = interp1d(
        xgrid, SI.e * electric_field_t_integral[:, state_index] / SI.hbar, kind="cubic"
    )
    mz = lambda x: effective_mass(mc, alpha, k_z(x))
    gx = lambda x: gam

    eps_x = create_eps_from(
        probe_om, interp1d(xgrid, state.number_density), lambda x: mc, gx
    )
