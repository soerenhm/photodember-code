...
def energy_current_density(u: ArrayF64, dx: float, e: float, T: ArrayF64, eF: ArrayF64, E: ArrayF64, L11: ArrayF64, L12: ArrayF64) -> ArrayF64:
    ...
    for i in range(1, len(u)):
        ...
        L12i = .5*(L12[i] + L12[i-1])
        u[i] = -.5*(L11[i]/T[i]+L11[i-1]/T[i-1])*T_dx + (L12i * (eF_dx / e + E[i]))
    return u
...
def charge_current_density(J: ArrayF64, dx: float, e: float, T: ArrayF64, eF: ArrayF64, E: ArrayF64, L21: ArrayF64, L22: ArrayF64) -> ArrayF64:
    ...
    for i in range(1, len(J)):
        T_dx = (T[i] - T[i-1])/dx
        eF_dx = (eF[i] - eF[i-1])/dx
        L22i = .5*(L22[i] + L22[i-1])
        J[i] = -.5*(L21[i]/T[i]+L21[i-1]/T[i-1])*T_dx + (L22i * (eF_dx / e + E[i]))
    return J
...
@numba.njit
def number_density_differntial(dN: ArrayF64, dt: float, dx: float, q: float, J: ArrayF64) -> ArrayF64:
...
@numba.njit
def energy_density_differential(dU: ArrayF64, dt: float, dx: float, u: ArrayF64, J: ArrayF64, E: ArrayF64) -> ArrayF64:
...
def heat_capacity(C: ArrayF64, N: StateFunction, U: StateFunction, T: ArrayF64, eta: ArrayF64) -> ArrayF64:
...
def jacobian_from_densities(jac: ArrayF64, N: StateFunction, U: StateFunction, T: ArrayLike, eta: ArrayLike) -> ArrayF64:
...
@numba.njit
def transform_density_differentials(dT: ArrayF64, deta: ArrayF64, jac: ArrayF64, dN: ArrayF64, dU: ArrayF64) -> Tuple[ArrayF64, ArrayF64]:
...
def fermi_energy(eF: ArrayF64, eta: ArrayF64, T: ArrayF64, kB: float, electric_charge: float) -> ArrayF64:
...
@numba.njit
def electric_field(E: ArrayF64, dx: float, eps_0: float, eps_rel: float, rho: ArrayF64) -> ArrayF64:
...
@numba.njit
def electric_potential(phi: ArrayF64, dx: float, eps_0: float, eps_rel: float, rho: ArrayF64) -> ArrayF64:
...
@numba.njit
def ambipolar_electric_field(E, dx: float, q: float, L21e, L21h, L22e, L22h, Te, Th, zeta_e, zeta_h):
    E[0] = 0.0
    for i in range(1, len(E)-1):
        ...
        E[i] = (.5*(L21e[i]/Te[i]+L21e[i-1]/Te[i-1])*dTe + .5*(L21h[i]/Th[i]+L21h[i-1]/Th[i-1])*dTh)/(L22ei+L22hi) - (L22ei*dzeta_e + L22hi*dzeta_h)/q/(L22ei+L22hi)
    return E

# -----------------------------------------------------------------------------
# Solve T, eta

def solve_reduced_chemical_potential(number_density_func: StateFunction, T: float, number_density: float, bounds: Tuple[float,float]) -> float:
    ...
    if not result.success:
        print(f"Minimization did not reach the desired tolerance; `eta` for `T = {T}` and `N = {number_density}` may be unreliable.")
    return np.atleast_1d(result.x)[0]
    
def solve_temperature(energy_density_func: StateFunction, eta: float, energy_density: float, bounds: Tuple[float,float]) -> float:
    f = lambda x: (np.log(energy_density) - np.log(energy_density_func(x, eta)))**2
    result = optim.minimize_scalar(f, bounds, method="golden", tol=1e-14)
    if not result.success:
        print(f"Minimization did not reach the desired tolerance; `T` for `eta = {eta}` and `U = {energy_density}` may be unreliable.")
    return np.atleast_1d(result.x)[0]