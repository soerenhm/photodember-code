...
class MultiLorentz:
    ...
    def dielectric_function(self, om, shift=0.0):
        eps = self.eps_inf + 0j
        return eps + sum(
            [t.dielectric_function(om, shift=shift) for t in self.oscillators]
        )
    ...
    def shift_bandgap(model: "MultiLorentz", shift: float) -> "MultiLorentz":
        return replace(
            model,
            oscillators=[
                Lorentz.shift_bandgap(osc, shift) for osc in model.oscillators
            ],
        )
...
class Drude:
    ...
    def dielectric_function(self, om: float, n: float) -> complex:
        ...
        return (
            eps_val + (1 - eps_val) * n / n_val - (om_p / om) * (om_p / (om + 1j * gam))
        )
...
class DrudeLorentz:
    ...
    def dielectric_function(
        self, om: float, n: Union[float, ArrayF64], shift: Union[float, ArrayF64] = 0.0
    ):
        m, n_val, gam = self.m, self.n_val, self.gam
        eps_rel = self.lorentz.dielectric_function(om, shift)  # type: ignore
        om_p = np.sqrt(SI.e**2 * n / (SI.eps_0 * m))
        return (
            eps_rel + (1 - eps_rel) * n / n_val - om_p**2 / (om**2 + 1j * om * gam)
        )
