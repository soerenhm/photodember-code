...
class CubicLogInterp:
    ...
    def eval(self, T: ArrayLike, eta: ArrayLike, dT=0, deta=0, grid=False) -> ArrayLike:
        ...
        if dT == 0 and deta == 0:
            ...
            raise NotImplementedError("Derivatives of order higher than 1 are not yet implemented!")
...
class CubicLogInterpFastDeriv:
    ...
    def interpolate(table: FermiDiracTable, property: str) -> CubicLogInterpFastDeriv:
        ...
        qty = np.log(table.as_array2d(property))
        spl = RectBivariateSpline(T, eta, qty, kx=3, ky=3, s=0.0)
        spl_dT = RectBivariateSpline(T, eta, spl(*grid, dx=1, grid=False), kx=3, ky=3, s=0.0)
        spl_deta = RectBivariateSpline(T, eta, spl(*grid, dy=1, grid=False), kx=3, ky=3, s=0.0)
...
def interpolate_particle_attributes(table: FermiDiracTable, electric_charge: float, kB: float, method: Interpolation=DEFAULT_FD_INTERPOLATOR):
